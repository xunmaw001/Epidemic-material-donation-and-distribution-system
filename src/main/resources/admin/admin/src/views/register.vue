    ...
    <div>
        <div class="container">
            <div class="login-form">
                <h1 class="h1" style="color:#000;fontSize:28px;">疫情物资捐赠和分配系统注册</h1>
                <el-form class="rgs-form">
                    ...
                    <el-form-item label="重复密码" class="input">
                        ...
                        <el-form-item label="机构姓名" class="input" v-if="tableName=='jigou'">
                            <el-input v-model="ruleForm.jigouName" autocomplete="off" placeholder="机构姓名"  />
                        </el-form-item>
                        <el-form-item label="机构手机号" class="input" v-if="tableName=='jigou'">
                            <el-input v-model="ruleForm.jigouPhone" autocomplete="off" placeholder="机构手机号"  />
                        </el-form-item>
                        <!--
                            <el-form-item v-if="jigouPhotoFlag">
                            </el-form-item>
                            <el-form-item label="机构头像" class="input" v-if="tableName=='jigou'">
                                    <file-upload
                                        tip="点击上传机构头像"
                                        action="file/upload"
                                        :limit="1"
                                        :multiple="true"
                                        :fileUrls="ruleForm.jigouPhoto?$base.url+ruleForm.jigouPhoto:''"
                                        @change="jigouPhotoUploadChange"
                                    ></file-upload>
                           </el-form-item>
                        -->
                        <el-form-item label="机构邮箱" class="input" v-if="tableName=='jigou'">
                            <el-input v-model="ruleForm.jigouEmail" autocomplete="off" placeholder="机构邮箱"  />
                        </el-form-item>
                        <!--
                            <el-form-item label="创建时间" class="input" v-if="tableName=='jigou'">
...
<script>
    export default {
        ...
        methods: {
            ...
            jigouPhotoUploadChange(fileUrls) {
               this.ruleForm.jigouPhoto = fileUrls;
                if(this.jigouPhotoFlag){
                    this.jigouPhotoFlag=false;
                }else{
                    this.jigouPhotoFlag=true;
                }
            },
            ...
            login() {
                            ...
                            if((!this.ruleForm.jigouName)&& 'jigou'==this.tableName){
                                this.$message.error('机构姓名不能为空');
                                return
                            }
                             if('jigou' == this.tableName && this.ruleForm.jigouPhone&&(!this.$validate.isMobile(this.ruleForm.jigouPhone))){
                                 this.$message.error('手机应输入手机格式');
                                 return
                             }
                            // jigou 中的 机构头像 判空处理
                            // if('jigou' == this.tableName && (this.ruleForm.jigouPhoto== null ||this.ruleForm.jigouPhoto== "")){
                            //     this.$message.error("机构头像不能为空");
                            //     return
                            // }
                            if('jigou' == this.tableName && this.ruleForm.jigouEmail&&(!this.$validate.isEmail(this.ruleForm.jigouEmail))){
                                this.$message.error("邮箱应输入邮件格式");
                                return
                            }
                            // jigou 中的 创建时间 判空处理
                            // if('jigou' == this.tableName && (this.ruleForm.createTime== null ||this.ruleForm.createTime== "")){
...
<style lang="scss" scoped>
	...
	.container {
		...
		background-image: url(/wizijuanzenghefenpei/img/back-img-bg.jpg);
